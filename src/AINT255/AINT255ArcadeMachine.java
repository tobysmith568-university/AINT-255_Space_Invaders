 ...
 */
public class AINT255ArcadeMachine {
     ...
     */
    public static double[] runOneGameAINT255(String game_file, String level_file, boolean visuals, String agentNames,
            String actionFile, int randomSeed, int playerID, AINT255MLPController mlpController) {
...
//        }
        ...
        // Warm the game up.
        AINT255ArcadeMachine.warmUp(toPlay, CompetitionParameters.WARMUP_TIME);
        ...
        for (int i = 0; i < no_players; i++) {
            ...
            if (no_players > 1) {
                // multi player
                players[i] = AINT255ArcadeMachine.createMultiPlayer(names[i], actionFile, toPlay.getObservationMulti(i),
                        randomSeed, i, humans[i]);
            } else {
                // single player
                players[i] = AINT255ArcadeMachine.createPlayer(names[i], actionFile, toPlay.getObservation(), randomSeed,
                        humans[i]);
            }
        ...
        mlpAgent = (MLPAgent) players[0];

        mlpAgent.setMlpController(mlpController);
        ...
        AINT255ArcadeMachine.tearPlayerDown(toPlay, players, actionFile, randomSeed, true);
     ...
     * @param level_file file with the level to be played.
     * @param visuals true to show the graphics, false otherwise.
     * @param actionFile name of the file where the actions of these players,
     * for this game, must be read from. If the game is multi player, this file
     * contains meta game information (winner, scores, timesteps, random seed)
     * and names of all the files for player actions.
     *
     */
    public static double[] replayGame(String game_file, String level_file, boolean visuals, String actionFile) {
        ...
        for (int i = 0; i < no_players; i++) {
            if (no_players > 1) {
                // multi player
                players[i] = AINT255ArcadeMachine.createMultiPlayer(agentName, null, toPlay.getObservationMulti(i), -1, i,
                        false);
            } else {
                // single player
                players[i] = AINT255ArcadeMachine.createPlayer(agentName, null, toPlay.getObservation(), -1, false);
            }
        ...
        AINT255ArcadeMachine.tearPlayerDown(toPlay, players, actionFile, seed, false);
     ...
     * @param actionFiles names of the files where the actions of this player,
     * for this game, should be recorded. Accepts null if no recording is
     * desired. If not null, this array must contain as much String objects as
        ...
        for (String level_file : level_files) {
            for (int i = 0; i < level_times; ++i) {
                ...
                for (int j = 0; j < no_players; j++) {
                    if (no_players > 1) {
                        // multi player
                        players[j] = AINT255ArcadeMachine.createMultiPlayer(agentNames[j], filename,
                                toPlay.getObservationMulti(i), randomSeed, j, false);
                    } else {
                        // single player
                        players[j] = AINT255ArcadeMachine.createPlayer(agentNames[j], filename, toPlay.getObservation(),
                                randomSeed, false);
                    }
                    score[j] = -1;
                    if (players[j] == null) {
                ...
                // down.
                if (!AINT255ArcadeMachine.tearPlayerDown(toPlay, players, filename, randomSeed, true)) {
                    score = toPlay.handleResult();
                    toPlay.printResult();
                }

...
public abstract class Game {
     ...
     * @param itype main int type of this sprite (leaf of the hierarchy of
     * types).
     */
    ...
    public double[] playGame(Player[] players, int randomSeed, boolean isHuman, int humanID) {
        ...
        for (Player player : players) {
            if (player instanceof AINT255.HumanAgent) {
    ...
    /**
     * Prints the result of the game, indicating the game id, level id, winner,
     * the score and the number of game ticks played, in this order.
     */
     ...
     *
     * @param players the players that will play the game (only 1 in single
     * player games).
     */
    ...
    protected void clearAll(ForwardModel fm) {
        for (VGDLSprite sprite : kill_list) {
            ...
            if (sprite.is_avatar) // go through all avatars to see which avatar is dead
            {
                for (int i = 0; i < no_players; i++) {
                    if (sprite == avatars[i]) {
                        avatars[i] = null;
     ...
     * @param transformed - indicates if the sprite was transformed (necessary
     * to kill sprite even if avatar, instead of disabling it).
     */

...
public class ForwardModel extends Game {
     ...
     * visibleList[playerID][spriteType]==true : sprite.hidden[playerID] =
     * false;
     */
    ...
    final public void update(Game a_gameState) {
        ...
        for (int i = 0; i < spriteGroups.length; ++i) {
            ...
            /**
             * Index in the sprite group passed to the checkSpriteFeatures
             * method to identify player in case of avatar sprites (index used
             * as playerID in the avatars array).
             */
    ...
    /**
     * Gets the sprite observation of a given sprite. Creates it if id didn't
     * exist.
     *
    ...
    /**
     * Initializes the variables of this game that have always a determined
     * value at the beginning of any game.
     */
    private void init() {
    ...
    /**
     * Initializes the non volatile elements of a game (constructors,
     * termination conditions, effects, etc). 'this' takes these from
     * a_gameState,
     *
    ...
    /**
     * Returns the sampleRandom generator of this forward model. It is not the
     * same as the sampleRandom number generator of the main game copy.
     *
     ...
     *
     * @param actions array of actions of all players (index in array
    ...
    /**
     * Method overloaded for multi player games. Gets the game score of a
     * particular player (identified by playerID).
     *
    ...
    /**
     * Method overloaded for multi player games. Indicates if there is a game
     * winner in the current observation. Possible values are
     * Types.WINNER.PLAYER_WINS, Types.WINNER.PLAYER_LOSES and
     * Types.WINNER.NO_WINNER.
     *
     * @return the winner of the game.
     */
     ...
     * guarantee that this position reflects the real position of the avatar
    ...
    /**
     * Returns the speed of the avatar. If the game is finished, we cannot
     * guarantee that this speed reflects the real speed of the avatar (the
     * avatar itself could be destroyed). If game finished, this returns 0.
     *
     ...
     * guarantee that this orientation reflects the real orientation of the
     * avatar (the avatar itself could be destroyed). If game finished, this
     * returns Types.NIL.
     *
    ...
    /**
     * Returns the actions that are available in this game for the avatar. If
     * the parameter 'includeNIL' is true, the array contains the (always
     * available) NIL action. If it is false, this is equivalent to calling
     * getAvailableActions().
     *
     * @param includeNIL true to include Types.ACTIONS.ACTION_NIL in the array
     * of actions.
    ...
    /**
     * Returns the resources in the avatar's possession. As there can be
     * resources of different nature, each entry is a key-value pair where the
     * key is the resource ID, and the value is the amount of that resource type
     * owned. It should be assumed that there might be other resources available
     * in the game, but the avatar could have none of them. If the avatar has no
     * resources, an empty HashMap is returned.
     *
     ...
     * ACTION_NIL. Note that this may NOT be the same as the last action given
     * by the agent, as it may have overspent in the last game cycle.
     *
    ...
    /**
     * Returns the avatar's type. In case it has multiple types, it returns the
     * most specific one.
     *
     ...
     * mean that the avatar is dead (could be that no health points are in use
     * in that game).
     *
    ...
    /**
     * Method overloaded for multi player games, returns true if the avatar is
     * still alive.
     *
     * @param playerID ID of the player to query.
     */
     ...
     * considered.
     * @return List of arrays with Observations. Each entry in the array
     * corresponds to a different sprite type.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns a list of observations of NPC in the game. As there can be NPCs
     * of different type, each entry in the array corresponds to a sprite type.
     * Every ArrayList contains a list of objects of type Observation, ordered
     * asc. by distance to the avatar. Each Observation holds the position,
    ...
    * @return a list with observations of resources.
    ...
    /**
     * Returns a list of observations of objects created by the avatar's
     * actions.
     *
     * @param refPosition Reference position to use when sorting this array, by
     * ascending distance to this point.
